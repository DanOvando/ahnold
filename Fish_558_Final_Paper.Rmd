---
title: "The Effects of the Marine Life Protection Act on Fish Densities in the Channel Islands"
author: "Dan Ovando"
...
In addition, the MPAs were designed in with the goal of achieving promoting conservation while supporting livelihoods. In addition to the review process, models were used help design the MLPA reserves that explicitly sought to examine the trade-offs and outcomes for conservation and fishing as a result of MPA network configurations (though these models were not explicitly incorporated into the planning process) [@Rassweiler2012,@Costello2010]. Empirical analysis such as this allow us to begin evaluating to what extent these design objectives are being realized. 
...
### Likelihoods
  ...
  For the logit component of the model we can tally the number of candidate transects *n* and the number of transects with positive densities *w* that comprised that density estimate. This translates to a binomial process where the with a probability density given by
...
We also see that substantial auto-correlation remains in the samples, evening after thinning 5e6 iterations down to 1,000. Observing the lag-1 auto-correlation of all `r dim(processed_demon$thinned_post)[2]` parameters, we see that a substantial number of parameters have substantial autocorrelation, including near perfect autocorrelation in for many variables (Fig.6).
...
We see that the observed test statistics fall within the 95% credibility interval of the test statistics generated from the posterior predictive for all statistics except the max (Fig.9). This suggests that the model is failing to predict the higher values of observed densities. We can confirm this bias by examining the observed densities plotted against the median posterior predicted value for a given observation, where we see a strong trend towards under-predicting higher densities (Fig.10). This bias is less a concern for this model since we are more concerned with the robustness of the DiD estimators than the predictive ability, but is worth noting. We could explore a log-normal or other such distribution with a longer right-hand tail to see if they are better able to capture the higher observed densities. 
...
check <- subset(den_post_preds, observation %in% samps) %>%
  ...
  post_summary_plot <- post_summary %>%
    ...
    ggplot(aes(value)) + 
    geom_histogram() + 
    geom_vline(aes(xintercept = c(lower95,upper95))) + 
...
Our initial goal was to attempt to detect the causal effect of the MLPA on fish densities throughout the Channel Islands region. Preliminary results indicate that:
  ...
  1. All else equal, the model identifies the contribution of the MLPA as having reduced densities of unfished species, with no detectable marginal effect on the densities fished species.  of unfished species throughout the Channel Islands
  2. The model suggests that the MLPA has reduced the probability of a given observation of a species' density at a site-side in a given year being zero. This could be attributed to increases in abundance, or potentially changes in sampling ability
...
So, this suggests that inclusion our hierarchical priors is not substantially influencing our point estimates of coefficients. It also suggests that while our MCMC has not converged, we would likely not expect its final values to differ substantially.  It also calls into question the next phase of this analysis, seeing as the frequentist version takes three lines of code and runs in less than one second, while the Bayesian approach is currently running for nine hours on the NCEAS server. Once we extend this analysis to multiple regions, or attempt to link the model to a more explicit model of the data collection process, or link to model estimates of spatial fishing patterns, the Bayesian framework may still prove useful. 
