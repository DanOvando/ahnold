...
```{r summary maps, cache=T, fig.cap= 'Map of survey locations. Color corresponds to year site was made an MPA. Size is proportional to area, though area data is missing for coastal sites'}
...
$$ D_{f,s,t} = \beta_{0} + MPA(1,0) + targeted(1,0) + factor(year) + factor(site)
...
species.siteside.year <- processed.dat %>%
  ...
  summarise(mpa.period = unique(period), density = sum(biomass, na.rm = T),
...
mutate(log.density = log(pmax(density, .0000001)),
...
```{r run regression}
  species.siteside.year.reg <- lm(log.density ~  factor(year)  + factor(site.side) + factor(trophic.group) + fishable + years.mpa + MPA + MPA:eventual.mpa , data = subset(species.siteside.year, density >0))
