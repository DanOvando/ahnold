...
This paper utilizes econometric techniques to ask: what effect has the California Marine Life Protection Act had on fish densities in the Channel Islands National Marine Sanctuary? The Marine Life Protection Act (MLPA) created a network of no-take marine protected areas (MPAs) reserves in the Channel Islands (CI) in 2003. We propose to use a database of visual fish abundance surveys that have run in the CIs since 1999 to assess what effect the MLPA has had on commercially fished and unfished species throughout the Islands. 

At its face this question may not appear particularly novel. The concept itself is rather obvious: Stopping fishing in an area should increase the abundance of species that used to be fished inside that area. Many studies have documented increases in numbers, biomass, species richness, and other metrics inside the borders of protected areas [@Lester2009, @Halpern2003], though it should be noted that these results are mostly correlational. We propose two novel extensions. First, we utilize a difference-in-difference technique to attempt to assign a causal effect to the MLPA. Second, we extend our analysis outside the borders of the MPAs, to isolate the effect of the MLPA on the Channel Islands as a whole. 

In addition, the MPAs were designed in with the goal of achieving promoting conservation while supporting livelihoods. In addition to the review process, models were used help design the MLPA reserves that explicitly sought to examine the tradeoffs and outcomes for conservation and fishing as a result of MPA network configurations (though these models were not explicitly incorporated into the planning process) [@Rassweiler2012,@Costello2010]. Empirical analysis such as this allow us to begin evaluating to what extent these design objectives are being realised. 
...
For these two reason, we use whether or not a given species is targeted by fishing, commercially or recreational, in Southern California. Under this experimental structure, the application of the MLPA can be thought of as an exogenous shock to the fish species, some of which were fished and some of which were unfished. While it is certainly true that MPAs may have been applied in sites with higher (or lower) concentrations of fished species, at the level of the Channel Islands species certainly could not themselves influence the creation of the MLPA, or select whether or not to be in the Islands at all. Therefore, the application of the MLPA on fished or unfished groups serves as a rough approximation of an exogenous shock to the species living in the Channel Islands (though we discuss potential limitations to this assumption in the discussion). Examining the data, we see a relatively even distribution of the percentage of species observed at a site targeted by fishing, overall 35% targeted in non-MPA sites and 36% targeted in MPAs (Fig.XX). 
...
perc_targeted_plot <- species_siteside_year %>%
 ...
 perc_targeted_summary<- species_siteside_year %>%
  group_by(site_side) %>%
  summarize(perc_targeted = mean(targeted == 'Targeted'), MPA = unique(eventual_mpa)) %>%
  group_by(MPA) %>%
...
dense_hist <- species_siteside_year %>%
...
$$\gamma_{f,s,y} = \tau_{0} + \tau_{1}$$
  ...
  We include the DiD estimator within the hurdle portion of the model to estimate the effect of the MLPA on the probability of zero-density observations.

For the observations with *d* > 0, we model the expected density as

$$ \hat{d_{f,s,y}} = \beta_{1}fished_{f} + \beta_{2}mpa_{s,y} + \beta_{3}FxM_{f,s,y} +
...
### Likelihoods
  ...
  For the hurdle component of the model we can tally the number of candidate transects *n* and the number of transects with positive densities *w* that comprised that density estimate. This translates to a binomial process where the with a probability density given by

$$ll^{h}_{f,s,y} = log(\left(
    \begin{array}{c}
      n_{f,s,y} \\
      o_{f,s,y}
    \end{array}
  ...
  where *p* is the estimated probability of the the number of positive densities and *ll* is the log likelihood of a given observation for hurdle *h* or density *d* for the positive density model. 
...
We can further examine the convergence of the density DiD estimators by examining the partial-chain comparisons (Fig.XX) and Heidelberger-Welsh statistics for the DiD estimator parameters. The "fished" and "years MLPA" coefficients both pass the Heidelberger-Welsh test, but the interaction term (fished x YearsMLPA) did not. Interestingly, examining the partial chain comparisons, we the exact opposite result: the complete and partial chains of the "fished" and "years MLPA" do not match, while the partial chain comparison of the interaction term appears stable (Fig.XX). Considering the DiD estimators for the hurdle component of the model, only the interaction term passes the Heidelberger-Welsh test. As with the density regression, only the interaction term in the hurdle model component appears to indicate stability in the chain (Fig.XX).
...
We also see that substantial auto-correlation remains in the samples. Observing the lag-1 auto-correlation of all `r dim(processed_demon$thinned_post)[2]` parameters, we see that a substantial number... (Fig.XX).
...
We also see that due to this auto-correlation issues, the effective sample size of the vast majority of our samples are incredibly small. This further supports that so far many of the coefficients of the model are very poorly estimated. 
...
We see some explanation for this behavior by examining the cross-correlation plot of the estimated coefficients. Specifically, we see very high correlation among the year and region fixed effects. We see that the year terms are almost perfectly positively correlated. The year terms are almost perfectly negatively correlated with the region fixed effects as well. All this suggests that we may need to more directly account for the covariance among the coefficients. 

```{r crosscorr, fig.cap='Cross-correlations of model coefficients. bi indicates components used the the binomial hurdle model'}
...
Moving to the posterior predictives, we see that the medians of the posterior predictive distribution of densities do not capture the observed densities very well (Fig.XX). In fact xxx

```{r post_predict, fig.cap='Observed vs. median posterior predicted'}
plots$fit_plot

processed_demon$resids

```

The preceding MCMC diagnostics provide evidence that we will need to perform substantially more iterations than my computer and timeline can handle at this moment. However a XX million iteration will be running during my drive to Santa Barbara. Beyond the MCMC diagnostics, we should consider standard regression diagnostics to understand the validity of our coefficients. For now we focus on the density model. Our assumption is that the log-densities are normally distributed. Looking at a normal quantile-quantile plot of the residuals of the density model, we see that our assumption of normality appears justified. 
...
We can now turn to the nested nature of the data. We know that the data are clustered by year, site-side, and species. Looking at the residuals by time, it appears that the inclusion of year fixed effects and lagged temperature effects were sufficient to remove heteroskedasticity and correlation in the residuals over time (Fig.XX)

```{r, fig.cap = 'Residuals by time'}
plots$resids_by_time_plot
```

Turning to the regional effects, we would expect there to be correlation in the residuals among nearby sites. We do not include site-specific fixed effects as the model quickly becomes over-fit at that point. We do include region fixed effects with a hyperprior in an effort to control for spatial correlations in densities. We see that while there is clear correlation in the residuals, it appears that the region effects were able to substantially account for this (Fig.XX). 
...
Lastly, we can consider the residuals by species. We might assume that the life history characteristics included in the model would be best represented by a multivariate normal distribution, as clearly parameters such as *L~inf~* and *k* are correlated. However we have not yet accounted for this, partly to reduce the complexity of the model at this stage. Examining the residuals by species group, we can see that there is clear within-species correlation in the residuals, though there does not appear to be within tropic-group correlation. This indicates that while the degree of spatial correlation does not appear severe enough to warrant more complex modeling of spatial correlations, we will need to address the within-species correlations. 
...
plots$outs_of_interest_sigma

```


Lastly, we can consider this process as roughly analogous to a CPUE standardization in the "delta" manner summarized by Maunder & Punt [-@Maunder2004]. From that perspective, the coefficients of the year fixed effects in the density model (which are scaled relative to the year 2000) being the standardized indices of abundance. From this perspective, we see an interesting (and troublesome) trend of increasing densities post 2003, coincidentally the same year as the MLPA. 
...
Our initial goal was to attempt to detect the causal effect of the MLPA on fish densities throughout the Channel Islands region. Preliminary results indicate that:
  ...
  1. All else equal, the model identifies the contribution of the MLPA as having reduced densities. In the specific interpretation of the fixed effects, the data suggests that the MLPA has reduced densities of unfished species throughout the Channel Islands.
  2. We can detect no significant effect of the MLPA on fished species throughout the region
...
This is clearly an initial investigation and it remains to be seen if these results hold. There are several key issues that need to be investigated. First examining Fig.XX, we see that the trend in abundances, as measured by the coefficients of the year fixed effects, shows an general increase beginning in and continuing since 2003, the year in which the MLPA MPAs were implemented in the Channel Islands. Given that we only have data from the Channel Islands at this point, we have no "contrast" in the number of years protected by the MLPA: all regions start in 2003. As such, it is very difficult for the model to separate out the effects of the MLPA from the year fixed effects. We can verify this by examining Fig.XX, which shows that year effects and the DiD estimators related to years protected by the MLPA are highly correlated. This suggests that obtaining robust estimates of the individual coefficients of each term may be challenging. Data from the south and central coast, which have different start-dates for the MLPA, should help provide much clearer contrast in the data to separate out the effects of the MLPA from time-trends. 

Further consideration of the hierarchical nature of the data also warrants consideration. While our results show that we have accounted for spatial and temporal correlations reasonably well, we have not properly accounted for the within-species nesting of the data. However, we would expect that heteroskedasticity at the species level would not bias our results, but would be more likely to influence the distribution around the expected value.  

As some support for the notion that we may not need to be as concerned with the nested nature of the data in order to obtain expected values of coefficients, we compared the estimates obtained from the Bayesian regression described here, to an ordinary least squares estimate of the coefficients for the same parameters. We see that by and large, and very depressingly, the two methods yield nearly identical results for most of the coefficients in the model. So, this suggests that inclusion our hierarchical priors is not substantially influencing our point estimates of coefficients. It also calls into question the next phase of this analysis, seeing as the frequentist version takes three lines of code and runs in less than second, while the Bayesian approach is currently running for nine hours on the NCEAS server. Once we extend this analysis to multiple regions, or attempt to link the model to a more explicit model of the data collection process, or link to model estimates of spatial fishing patterns, the Bayesian framework may still prove useful. 

From the perspective of causality, the inclusion of the DiD estimator gives us greater confidence that we are estimating the causal effect of the MLPA, as opposed to inside-outside or fished-unfished comparisons. In addition, by including a representative suite of plausible covariates we likely improve the point estimates of the coefficients, since omission of factors such as the life history of species or time trends are highly likely to suffer from omitted variable bias. 

Substantial work remains though before we can be certain that we have reasonable identification of the causal effect of the MLPA. First, we should still consider more rigorous model selection, possibly through reversible jump MCMC, or through frequentist or non-parametric methods. Second, at the moment we only include two "groups" for our DiD estimator: fished or unfished. The simple DiD estimator employed here works best with >30 groups, for example examining 30 different cities, some of which were exposed to a treatment. As a first step our simple approach is ideal to explore the data, but to identify causality we should explore strategies for improving this. One alternative may be to break fished and unfished into groups of treatment level (e.g. degree to which they are fished), or into groups of species categories. However both of these strategies will increase the number of model parameters, and the strong correlations between many of the included variables suggests that we may have trouble obtaining reliably parameter estimates with many more fixed effects. Alternatively, bootstrapping methods exist for the DiD estimator when the number of groups is small. 

The DiD estimator should become more robust once we can add additional regions and obtain contrast in the year of MPA implementation. Given that the data are noisy though we should not put all our eggs in one basket. One of the next steps is to explore fitting the same model using propensity scores based on species traits. This provides us another avenue towards identification, which we can compare to the DiD estimator. 

As a final consideration, it is worth noting what a poor predictor of densities our model is (Fig.XX). This is perhaps not surprising, given the relatively fine scale resolution of the data (species level along a transect of 30 meters), but is still worth considering. From a general perspective, this indicates the complicated evolution of fish populations over time, which those of us who engage in simulation studies of MPA effects should take seriously consider. 

Our preliminary results then do not detect a significant causal effect of the MLPA on the positive densities of fished species throughout the Channel Islands. While substantial work remains before we can hang our hat on this result, it does have one clear implication: the signal of the MLPA on regional fish densities is far from clear. While within-MPA results may show stronger contrast, we might not expect improved statistical methods to identify much clearer signals in the regional effects of the MLPA (though improved isolation of the MLPA effects and the year terms may yield dramatically different results). Given that the link between the MLPA and fish densities is relatively simpler than the link between the MLPA and fishing profits, it will be interesting to see how that next phase of the analysis goes. 
