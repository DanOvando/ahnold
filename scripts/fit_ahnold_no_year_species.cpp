...
Type objective_function<Type>::operator() ()
{
  ...
  matrix<Type> log_density_hat = non_nested_effects + did_effects + region_cluster_effects ; // + year_species_effects + region_cluster_effects;
  ...
  matrix<Type> logit_scale_prob_seeing = seeing_non_nested_effects + seeing_did_effects + seeing_region_cluster_effects; //+ seeing_year_species_effects + seeing_region_cluster_effects;
  ...
  // predict with mpa
  vector<Type> with_mpa_logit_standardized_yearly_prob_seeing = standard_non_nested * seeing_non_nested_betas + standard_region_cluster * seeing_region_cluster_betas +  standard_did_with_mpa * seeing_did_betas; //

  vector<Type> standardized_yearly_prob_seeing = 1 / (1 + exp(-with_mpa_logit_standardized_yearly_prob_seeing));

  vector<Type> with_mpa_standardized_abundance = standard_non_nested * seen_non_nested_betas  + standard_region_cluster * seen_region_cluster_betas +standard_did_with_mpa * seen_did_betas;

  vector<Type> log_abundance_with_mpa = standardized_yearly_prob_seeing * with_mpa_standardized_abundance;

  // predict without mpa
  vector<Type> without_mpa_logit_standardized_yearly_prob_seeing = standard_non_nested * seeing_non_nested_betas  + standard_region_cluster * seeing_region_cluster_betas+  standard_did_without_mpa * seeing_did_betas;

  standardized_yearly_prob_seeing = 1 / (1 + exp(-without_mpa_logit_standardized_yearly_prob_seeing));

  vector<Type> without_mpa_standardized_abundance = standard_non_nested * seen_non_nested_betas + standard_region_cluster * seen_region_cluster_betas + standard_did_without_mpa * seen_did_betas;
