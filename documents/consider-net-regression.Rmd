...
a <- abundance_indices %>%
  ...
  mutate(catch = ifelse(is.na(catch), 0, catch),
         post_mpa = year >=2003,
         temp_dev = abs(mean_temp - temperature) ) %>% 
  mutate(temp_dev = center_scale(temp_dev),
         mean_vis = center_scale(mean_vis),
         mean_canopy = as.factor(mean_canopy))
...
seen_model <- rstanarm::stan_glm('log_density ~  targeted + factor_year + targeted:factor_year + mean_enso + temp_dev + mean_vis + trunc_observer + site_side', data = seen_data, chains = 1, cores = 1)
