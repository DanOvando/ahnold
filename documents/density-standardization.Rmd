...
abundance_models <- length_to_density_models %>% 
  ...
  filter(str_detect(commonname, 'YOY') == F, is.na(targeted) == F) %>% # get rid of young of the year and unknown if targeted species
...
wtf <- vast_abundance$vast_data[[1]] %>% 
  group_by(year) %>% 
  summarise(nobs = sum(catch_kg > 0),
            nzero = sum(catch_kg == 0))
