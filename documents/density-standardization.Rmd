...
raw_length_vars <- paste(c('factor_year','observer', 'site','mean_vis','mean_canopy'), collapse = '+')

supplied_density_vars <- paste(c('factor_year', 'site','mean_kelp', 'mean_vis'), collapse = '+')
...
abundance_models <- length_to_density_models %>% 
  bind_rows(supplied_density_models) %>% 
    mutate(data = map(data, ~ filter(.x, year > 1999))) %>% 
...
model_plots <- consistent_models %>% 
  mutate(standardized_abundance_trend = map(abundance_index, ~select(.,factor_year,abundance_index) %>% mutate(year = as.character(factor_year) %>% as.numeric()))) %>% 
...
walk2(model_comparison_plots$commonname, model_comparison_plots$comp_plot,
      ~ ggsave(file = paste0(.x,'-modelcomp.pdf'), .y))
