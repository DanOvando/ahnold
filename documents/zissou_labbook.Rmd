...
So what if you said get the standardized index of abundance in the above manner. Then, estimate the DiD on those terms, controlling for things that were omitted from the abundance index, e.g. el nino, water temperature, etc. that’s pretty fucking good. 
...
THeory says we should see a specific effect

People think we should see this effect

When could we expect to detect to detect this
  
  - When does degree of overfishing swamp environmental noise
  
What does this look like in a real dataset

  - Evidence the even in this case, we can't detect it

# 2018-02-12 Checking with Olivier

Parallel the simulation process with the empirical. Track changes in results from least-to-most complex model. Revisit a bit later to make this automated. 

# Chapter push

What needs to happen to make ahnold chapter ready. 

1. What does theory tell us should be the expected network wide effect size of MPAs

  - Simple example
  - Use your simlator to present a histogram of expected MPA effects, plot MPA effects as a function of other covariates 

2. Do a simple power analysis to say: what kind of sample size/variance would you need to detect this effect?

3. Given that, let's look into realistic scenarios and see: can we detect the true effect (using your proposed control)

4. Crank up the weirdnessL when do things break down/how overfished do things need to be accurately detect the effect?

5. Demonstrate with MLPA data

6. Tease next steps: stock assessment MPA detection, synthetic controls?

# There's no damn reason to estimate an annual MPA effect: let's do pre, first five years, next five years

# simple power analysis idea

the idea is that you are sampling from an unfished and a fished, or an MPA and a no-MPA world. You want to say, can you say that the mean of the MPA world is substantially different than the no-mpa world, where each sample is a fraw of an MPA effect for that species 

Think of it like after ten years, where each draw is a sample MPA effect for one species at different places, or multiple species

```{r}

experiment <- data_frame(no_mpa =  rnorm(100,0.5,1), mpa = rnorm(100,0.5 * 1.1,1)) %>% gather(experiment, mpa_effect)

experiment %>% 
  ggplot(aes(mpa_effect, fill = experiment)) + 
  geom_density()
...
# thoughts on out of rabbit hole

f the DiD. What if you estimate a species specific MPA effect, which comes from a distribution of mean MPA effect, and that's the thing that you actually want to estiamte. 

Here's how you do that. Random year block effects for each species, where each year block for each species comes from a fished or an unfished distribution. i.e. the mean density of a species in a year block is a draw from a mean density for that species group (fished or unfished) in that year. The MPA effect then is the difference in the group level means in each year block (targed and non-targered). 

Need to think about whether this counts as a DiD or something else, once you include fixed or random effects by species 

no that does it! As long as you include intercepts for each species, the the year effects are the trends around that mean. So, by parallel trends if there's no effect then the difference mean trend should be zero, and then deviate after the MPAs. BOOYA. 

Shit, on a related note, I don't think your approach to the net delta effect is right: it does depend on the scale of the other coefficients, so it it can shrink or expand the average effect.... poop. 

Ask jim. I think you just need to get away from the zeros, and fit the model to more aggregate densities... should think about how to do that. 

Doing the manual estimate of the "MPA" effect doesn't properly account for the zeros, since the difference btween MPA on and MPA off is context dependent: its effect on density can be bigger or smaller depending on the value of the other parameters. So, it you're taking the MPA effect on the densities from the linaer model and multiplying those by a vector of detection probabilities over time, you might be diluting the "true" MPA effect. 

So back to the two stage model. 

Goal: An estimated average effect of MPA on densities of fished species

1. The raw data are transet level (30x2 meter transect) counts of fish (species, length bin, numbers). These data are at "site-side" locations, e.g. yellowbanks-east (eastern side of the yellowbanks site). Most sites are sureyed a few times per year. Sites are located throughout the channel islands, but only a few have been consistently surveyed since 2000 (monitoring of Santa Rosa and San Miguel islands only began after 2003). Some sites are inside MPAs, others outside. All MPAs in places in 2003 except for Anacapa reserved, 1973. Available covariates include the observer, visibillity, surge, kelp cover, temperature, and depth. 

2. Identification strategy: Density of unfished species as control for density of fished species, following  difference-in-difference style approach.

3. Working backwards... suppose that you have densities over time for fished and unfished species. The simplest DiD would be targeted + year + targeted:year. We know that doesn't work here, for a variety of reasons. What I want is the average group-level effect. 

So, a regression that was random effects for each species, year-species random effects, where the prior for each year-species effect comes from a mean density for targeted or untargeted species in that year block. Focusing just there for a moment. 

If each species has an intercept that is its mean density, then the time trends should be deviations from that density with mean 0 (hopefully...). So, under the parallel trends assumption then, pre-MPA the fished and unfished species should have roughly equal mean year effects, though individual species can vary. You definitely can't be estimating year-species effects, unless you do the densities at very fine levels, since if you do it by island then that means that for every species you onlt have at most 4 replicates per year.

So, the tradeoff becomes in estimating a bunch of standardized densities for each species at really fine spatial scales, which decreases bias but increases variance, or aggregating up to a larger level (e.g. island). From there, you're back to a simpler regression, and I think something like a simple DiD, where the effect is the difference in means between the targeted and non-targeted groups. 
...
run_dir <- "v1.0"
load(
  file = here::here("results",run_dir,"abundance_data.Rdata"))



tester <- abundance_data$data[abundance_data$data_source == "pisco"][[1]] %>%
  group_by(year, classcode, targeted) %>% 
  summarise(mb = mean(density_g_m2),
            loo = mean(loo)) %>% 
  filter(is.na(targeted) == F) %>% 
  group_by(classcode) %>% 
...
tester %>% 
  ggplot(aes(year, log(mb), color = factor(targeted), group = interaction(classcode, targeted))) + 
  geom_line(show.legend = T)
  
simple <- lm(log(mb) ~ targeted + factor_year + targeted:factor_year + loo, data = tester %>% filter(year > 1999))

a = simple %>% 
  broom::tidy() %>% 
  filter(str_detect(term, "targeted:"))

test <- stan_glmer(log(mb) ~ (1|classcode) + (factor_year - 1|targeted) - 1, data = tester %>% filter(year > 1999), chains = 1)
...
tester %>% 
  group_by(year, targeted) %>% 
  summarise(mmb = mean(mb)) %>% 
  ungroup() %>% 
  ggplot(aes(year, log(mmb), color = factor(targeted))) + 
  geom_line()

```

Yep so that confirms the mixed effect approach to this. So, the question is just what scale do you want the densities to be at. Good to know I'm not a total idiot. So, the first goal is a CPUE standardization function at arbitrary spatial scales, maybe different scales for each species? So you'd ahve a strata column that says like species_spatial. Once you fit that, look at the outcomes and fit the model outside of TMB, then pass it back to TMB. 


On the also plus side, this thing is just about dead on the expected magnitude of effects... 



# zissou revamp

Calling it zissou now. 

- Use recipes to fix up the data processing a bit, center and scale things, filter down uncommon observers etc. 

- Resurrect two-stage model
...
Just eyeballing it, not a lot of evidence that you need to be doing different trends for different species in different regions. Stop tinkering with the regression. 

```{r}
raw <- pisco %>%
  group_by(year, classcode, targeted, region) %>%
  summarise(md = mean(density_g_m2)) %>%
  group_by(classcode, region) %>%
  mutate(smd = (md - mean(md))/(sd(md))) %>%
  ungroup() %>%
...
raw %>% 
  ggplot(aes(year, smd, color = region)) + 
  geom_line()+ 
...
IUCN 30% mandate as start of paper fodder. Not binding where CBD is technically binding (10%)

Huxley - skepticism the highest duty, blind faith 

See making waves: the science and politics of ocean protection

resilience and MPA literature

replication, randomization, control and treatment

essington 2006 pelagic ecosystem response to a century of commercial fishing

Take a look at that spatial closure 

# Simulation checklist

Ok, so what needs to happen here? You need to merge in both the CDFW and manually generated f series, a la `scrooge`. You also need to add in density dependent adult movement. I think those are the only bells and whistles that need to happen. 

Key experiment checklist:

- multiple species (mostly for aggregation later)

- depletion (0.25 increments between crazy overfished to unfished)

- steepness (0.6 and 1)

- larval movement (high and low)

- adult movement (high and low)

- adult density denepdent movement (on and off)

- recruitment form (for anything with steepness less than 1)
...
That's about  172,000 experiment run... not terrible but not great. Let's get one running and sus on the timing on this with and without open-access in there, and see if there's a better way to fix open access. 

The biggest overhead is doing this with 18 species, is that really needed? The advantage is that it allows you to more or less generate trajectories for the species in the islands. But if not, seems like grabing a few representative species would work just fine (do you really need 6 rockfish)

You forgot what you've done already. run-mpa-sims already did a lot of the legwork here. Now, you were also doing the actual regression analysis over in sim-mpa.R. 

So, the plan now is to first simulate all the experiments, and then to simulate data sampling from that library if you need.

 kelp rockfish ("satr") is driving a bit of the downturn in the fished species, a bit flatter without them. 
 
# taking a longer look at kfm vs pisco


```{r}

used <- pisco$classcode %>% unique()

kfm <- abundance_data$data[[2]] %>% 
  filter(classcode %in% used) %>% 
  group_by(classcode, commonname, year, targeted) %>% 
...
compare <- kfm %>% 
  bind_rows(p2) %>% 
  group_by(classcode, source) %>% 
  mutate(smd = (mean_density - mean(mean_density))/ sd(mean_density))

compare %>% 
  ggplot(aes(year, smd, color = source)) + 
  geom_line() + 
  facet_wrap(~classcode)
...
# why the zero adding process?

At the moment, you are adding in zeros for fish that have ever been seen at a site. What's the point of this, instead of either adding in zeros for every species in the database/by region/whatever?

My reasoning was that sites blink on and off. So, what I don't want to do is add in a bunch of implausible zeros all of a sudden. If we had consistent sampling of each site over time, or random sampling of suitable habitat, then including all zeros all the time wouldn't matter all that much. But, if you're measuring polar bear densities, and all of a sudden start including Miami in your density counts, that's going to screw up your time series. So, that's why it's site specific. 
...
# revisiting inside/outside MPA analysis

So both Ray and Steve want to see differences and density trends inside and outside of MPAs. 

First, can look at the data here, and compare trends and differentes just inside and just outside MPAs 

Second, could do it by island to see if there are differences in trends 

Third, if you want to, could identify knn in terms of lat/lon of nearest sites to each MPA

4th, chould sequentially add/drop species from the mix and see how much things change. KISS
...
cip_data <- pisco_data %>% 
  ...
  filter(region %in% c("ANA", "SCI","SRI",'SMI'),
         ...
         site %in% unique(consistent_sites$site))


inside_outside <- cip_data %>% 
  group_by(year, eventual_mpa) %>% 
  summarise(sum_density = sum(density_g_m2, na.rm = T),
            mean_density = mean(density_g_m2, na.rm = T),
            median_density = median(density_g_m2, na.rm = T))

inside_outside %>% 
  ggplot(aes(year, mean_density, color = eventual_mpa)) + 
  geom_line()


inside_outside_targeted <-  cip_data %>% 
  group_by(year, eventual_mpa, targeted) %>% 
  summarise(sum_density = sum(density_g_m2, na.rm = T),
            mean_density = mean(density_g_m2, na.rm = T),
            median_density = median(density_g_m2, na.rm = T))

inside_outside_targeted %>% 
  ggplot(aes(year, mean_density, color = eventual_mpa)) + 
  geom_line() + 
  facet_wrap(~targeted)
...
inside_outside_targeted %>% 
  ggplot(aes(year, mean_density, color = targeted == 1)) + 
  geom_line() + 
...
So the plot on the right there should mirror 


What about displaying the noise...
...
cip_data %>% 
  ...
  mutate(cs_density = (mean_density - mean(mean_density)) / sd(mean_density)) %>% 
  ungroup() %>% 
  ggplot(aes(year, mean_density, color = targeted, fill = targeted)) + 
  geom_line(aes(group = interaction(classcode, targeted)),alpha = 0.5) + 
  geom_smooth() +
  facet_wrap(~eventual_mpa)
```
...
Can you redo the MPA analysis using exact methods as jen for aggregation?


```{r}

a = cip_data %>% 
  group_by(targeted, region,site,side, transect, year, eventual_mpa) %>% 
  summarise(density = sum(density_g_m2)) %>% 
  group_by(targeted, region,site, year, eventual_mpa) %>% 
  summarise(mean_density = mean(density)) %>% 
  ggplot(aes(year, mean_density, color = targeted == 1, fill =  targeted == 1)) + 
  geom_smooth() + 
  facet_wrap(~eventual_mpa)
...
pop_depletion_and_size_plot <- outcomes %>% 
  ...
  group_by(experiment) %>% 
  mutate(f_v_m = plyr::round_any(f_v_m[years_protected == 0], .05),
         mpa_size = plyr::round_any(mpa_size, .05)) %>% 
  filter(years_protected == 15) %>% 
  group_by(f_v_m, mpa_size) %>% 
  summarise(median_mpa_effect = median(pop_effect)) %>% 
  ggplot(aes(f_v_m, mpa_size, fill = median_mpa_effect)) + 
  geom_tile() + 
  scale_fill_viridis(labels = percent,
                     ...
                     name = "Median Effect") +
  labs(x = "Pre-MPA F/M", y = "Range in MPA") + 
  scale_y_continuous(labels = percent) + 
  scale_x_continuous()


mpa_effects <- outcomes %>% 
  filter(years_protected >= -10) %>%
  group_by(experiment) %>% 
  mutate(f_v_m = plyr::round_any(mean(f_v_m[years_protected <= 0]), .05),
         mpa_size = plyr::round_any(mpa_size, .05)) %>% 
  filter(years_protected == 15) %>% 
  group_by(f_v_m, mpa_size) %>% 
  summarise(median_mpa_effect = median(mpa_effect),
            median_pop_mpa_effect = median(pop_effect)) %>% 
  ungroup()


f_and_size_plot <- mpa_effects %>% 
  ggplot(aes(f_v_m, mpa_size, fill = median_mpa_effect)) + 
  geom_tile() + 
  # geom_contour(aes(z = median_mpa_effect)) +
  scale_fill_gradient(low = "grey99", high = "grey15",labels = percent,
                     guide = guide_colorbar(frame.colour = "black",
                                            frame.linewidth = 1,
                                            barheight = unit(10,"lines")), 
                     name = "Median Effect") +
...
pop_f_and_size_plot <-mpa_effects %>% 
  ggplot(aes(f_v_m, mpa_size, fill = median_pop_mpa_effect)) + 
  geom_tile() + 
  # geom_contour(aes(z = median_mpa_effect)) +
  scale_fill_gradient(low = "grey99", high = "grey15",labels = percent,
                      guide = guide_colorbar(frame.colour = "black",
                                             frame.linewidth = 1,
                                             barheight = unit(10,"lines")), 
                      name = "Median Population Effect") +
  labs(x = "Pre-MPA F/M", y = "Range in MPA") + 
  scale_y_continuous(labels = percent)

save(file = "mpa_effects_for_ray.Rdata", mpa_effects, f_and_size_plot, pop_f_and_size_plot)
...
regional_pool <- (p1 + p2) * r * (1 - (p1 + p2)/ (2*k))


local_pool <- (p1 * r * (1 - p1 / k)) + (p2 * r * (1 - p2/ k))


growth <-
  data_frame(
  pop_structure = c("local", "regional"),
  growth = c(local_pool, regional_pool)
  )
  

growth %>% 
  ggplot(aes(pop_structure, growth)) + 
  geom_col()
...
MPA effects are going to be hard to detect. Both because they are small and because noise gets in the way. So the paper is both the empirics and the simulation in one go. 




# departmental semindar blurb

Marine Protected Areas offering varying levels of protection (MPAs) cover between 3-7% of the worlds oceans, up from less than 1% at the turn of the century. The Convention on Biological Diversity calls for 10% of coastal waters to be protected inside MPAs by 2020, with other bodies calling for at least 30% of the oceans to be placed in MPAs. This expansion is driven by a desire to achieve conservation benefits inside and outside of MPA borders, as well as economic benefits through for example tourism and fisheries spillover. While the potential conservation effects of MPAs inside their borders are well demonstrated, the effects of MPAs on the broader ecosystems and fisheries around them are far less understood. As MPAs play an increasing role in marine resource management portfolios, it is critically important that we develop a better understanding of the magnitude and drivers of regional-scale MPA effects. To address this gap, this study examines two critical questions: 1) What do we expect the regional-scale conservation and fishery effects of MPAs to be and 2) When (and how) can we expect to detect these effects? We address these questions using a simulation analysis to frame the theoretical regional conservation and fishery impacts of MPAs. From there, we present an empirical assessment of the evidence for regional-level conservation effects of MPAs resulting from a network of closures put in the Channel Islands National Marine Sanctuary.

# notes from Jen's edits

Interesting, should you add in a scenario where you specify whether the MPA is a network or a blob? Need to double check how you're doing that at this point. 

Chamge MPA to marine reserve maybe

Make text and axes labels consistent

# march 2013 call

Currently at 7,414 words and 9 pages, so a lot ahs to go. Excluding figures and tables you're at 47,000 characters. 

https://www.pnas.org/sites/default/files/advanced-pages/iforc.pdf

An online   submission   tool   provides   authors   with   an   estimation  of  whether  their  manuscript  fits  within  the  PNAS  length requirements (see Length Estimate Guidelines and FAQ). When submitting tables, figures, and/or equations in addition to  text,  keep  the  text  for  your  manuscript  under  39,000  char-acters (including spaces) for 6-page articles and 65,000 for 10-page articles.

Manuscript  Length.PNAS  generally  uses  a  two-column  for-mat  averaging  67  characters,  including  spaces,  per  line.  The  maximum length of a regular research article is 6 pages (or 10 pages for PNAS Plus articles), including all text, spaces, and the number  of  characters  displaced  by  figures,  tables,  and  equa-tions.

Tends to emphasize the negative results over the bigger message. Still thinking about this from a single species standpoint. If you've got an array of species with a mix of characteristics, small negative-positive effects, potentially large positive effects. So, you can get the big benefits for the species when you have the biggest gain, with minimal losses. Most are going to be a mix of species. 

Can we rework the title? Might not be hard to detect in a place with a the med. Those aren't the places that should be the focus of MPAs in the first place. 

Needs a takehome message for PNAS, need a key result. 

1. Here's why no-one is ever able to detect regional effects. 
  - A table of the papers on detecting MPA outcomes
2. When would you be able to detect an effect? Only when it matters
  - if you detect an effect, it must mean there was a big impact
3. You implement a bunch of species, you're going to detect for some but not others

we're just now starting to think about regional effects

if here's how you want to do the system wide effect, here's how, and here's why it's hard

inside outside can't give us insight to this answer. You've also got an estimate of inside vs outside effect and show how different that is. 

a statement about where the next generation of MPA science. We have established that we can do inside anda outside measurements.If we're really thinking about network effects, then the way that we measure them in the past is wrong. Here's a roadmap to the future. 

What if we forget. Is an inside outside comparison an illustration of the system wide effects? Can we continue to rely on inside-outside comparisons. THe main message is you need more comprehensive studies. PLot of inside outside to regional effect. 

It becomes a valuable solution to an ensemble. The cool thing that comes out of the right analysis are also places where you're not going to get huge fisheries cost. 

What can be learned from inside outside


plot: inside outside wedge vs distribution of conservation and fishery gains

Got to be a new story. 

virtually never designed for one species. 

You're just redistributing where the fish are. 

Can make the point that inside outside ratio vs projected benefit, have a kind of ratio that suggest maybe system wide benefits. 

when and where are the response ratios 

Why you can't rely on inside outside ratios 

Saving that place or reef 

What if we said, the main way that people measure the eficacy of MPAs is thorugh inside and outside comparisons. To the extent that we're implementing MPAs for system wide benefits, when can we rely on inside-outside benefits, to generalize to the region. Two issues arrise 1) SUTVA, but we assume 2) that this has been done, how do you scale up unbiased inside outside to regional and 3) we show a practical approach. 

Two reasonably ways of looking at this. 

**The system wide effects of MPA Networks**

If you look at your axes of stock status, if you're managing your fisheries well, not going to see a fisheries benefit. 


## is this the right message for PNAS?

## Go through and suggest improvements to figures

## Identify critical additional analyses to run

## What can/needs to be removed

this is a biggy. Can obviously cut a lot down, I tend to be wordy, but still it's a lot of content. Fisheries is some low hanging fruit. You don't have an empirical example for it, and the paper would be a bit more streamlined without it. But, makes it a stronger reference I think with it, and was a lot of work... though could write an almost identical companion paper if we can get the damn logbook data for the Channel islands fisheries. 

## What are the critical technical details to include in methods


# version 2.0


  Marine Protected Areas (MPAs) with varying levels of protection cover between 3-7% of the worlds oceans, up from less than 1% in the year 2000. The Convention on Biological Diversity's Strategic Plan for Biodiversity  calls for 10% of coastal waters to be protected inside MPAs by 2020, with other bodies calling for at least 30% of the oceans to be placed in MPAs. This expansion is driven by a desire to achieve conservation benefits inside and outside of MPA borders, and in some cases produce economic benefits to fisheries. Despite the expanding role of MPAs in marine resource management, we lack a clear understanding of what the regional effects of these protected areas might be and how to detect them. We used a bio-economic simulation model to demonstrate how the regional effects of MPAs are affected by suite of theoretical drivers, and show how commonly used to measure MPA effectivness can fail to capture the the true regional impacts. Lastly, we present am empirical strategy for estimating regional MPA effects on biomass density using 16 years of data from inside and outside of MPAs within the Channel Islands, California, USA. We find no detectable conservation effect of MPAs on total biomass densities of targeted finfish throughout the study region. The next generation of MPA science must address the challenge of understanding and estimating the regional effects of MPAs.
  
  
  
# how ready are you to explore density ratios and gradients?

after loading data for paper, and pulling in a random experiment
...
a <- readRDS(here("results","v3.0","experiments","experiment_10210.rds"))

b <- a$mpa_experiment[[1]]$raw_outcomes


d <- b %>% 
  group_by(year,patch, experiment) %>% 
  summarise(mpa = unique(mpa),
            biomass = sum(biomass))


d %>% 
  ggplot(aes(patch, biomass, color = mpa)) + 
  geom_point() + 
...
OK so you've got the data to do this, but going ot take some wranglign from the experiments. 

Points to a couple of things you might want to do better in the actual experiments though. Really not liking these edge effects at all, adds an unneeded layer of complexity. Also, thought you were allowing for random MPA placement. Let's go back through and actually look through spasm and see what can be done here. Might be a good time to revisit how you run this whole thing. 


# what are you doing about the constant catch scenarios that crash the population?

after loading in processed grid

OK solutino, let it run, but filter out any runs that bave biomass less than min biomass before MPA year
...
woof <- wtf %>% 
  select(experiment, mpa_effect) %>% 
  unnest() %>% 
  group_by(experiment) %>% 
  mutate(s = `no-mpa` / max(`no-mpa`))

woof %>% 
  ggplot(aes(year,s, group = experiment)) + 
  geom_line()

a = woof %>% 
  group_by(experiment) %>% 
  summarise(mb = min(`no-mpa`)) %>% 
  ggplot(aes(mb)) + 
  geom_histogram()
...
a = spasm::sim_fishery(
  fish = temp$fish[[i]],
  fleet = temp$fleet[[i]],
  manager = create_manager(mpa_size = .8),
  sprinkler = temp$sprinkler[i],
  mpa_habfactor = temp$mpa_habfactor[i],
  sim_years = 50,
  burn_years = 1,
  num_patches = 50
)

plot_spasm(a, type = "totals")

```


# revisit simplified analysis

just ot have in back pocket, revisit the idea of a super simple difference in difference, aggregating up etc. 

Starting at the point of run_tmb in run_zissou.R

simplest possible idea: mean biomass density by targeted and non targeted over time
...
supersimple %>% 
  ggplot(aes(year, mean_density, color = targ)) + 
  geom_line()


supersimple_reg <- stan_glm(log(mean_density) ~ targ*mpa, data = supersimple)

plot(supersimple_reg)


```


so next step then, look at it over time. So now, you need multiple observations per time steps, so going to look at each classcode
...
sortasimple2_reg <- stan_glmer(log(mean_density + 1e-3) ~ targ*fyear + (1|classcode), data = sortasimple, cores = 4)

bayesplot::mcmc_areas(as.matrix(sortasimple2_reg),
                      regex_pars = ":fyear") + 
...
huh interesitng, didn't notice that before, sheephead and kelp bass seem to be heading up a bit more clearly in the recent years, while the other ones are headed down. 
...
sortasimple <- data %>%
  filter(classcode %in% c("pcla", "spul") |
           targ == "Non-Targeted") %>%
  group_by(year, classcode) %>%
  summarise(mean_density = mean(density),
            targ = unique(targ)) %>%
...
sortasimple_reg <-
  stan_glm(log(mean_density + 1e-3) ~ targ * fyear,
           data = sortasimple,
           cores = 4)
...
And one more time but now treating sites as replicates to increase sample size
...
sortasimple <- data %>% 
  group_by(year, classcode, region) %>% 
  summarise(mean_density = mean(density),
            targ = unique(targ)) %>% 
     mutate(mpa = year > year_mpa) %>% 
  ungroup() %>% 
...
test <- stan_glm(lifeExp ~ factor(year), data = gapminder)
...
a = outcomes %>% 
  ...
  ungroup() %>% 
  select(-experiment,-msy,-b_msy,-year,-mpa_size1,
         -`no-mpa`,-`with-mpa`, -years_protected,
         -b0,-depletion,-pop_effect,-scientific_name)
...
a = outcomes %>% 
  ...
  select(-experiment,-msy,-b_msy,-year,-mpa_size1,
         -`no-mpa`,-`with-mpa`, -years_protected,
         -b0,-depletion,-mpa_effect,-scientific_name,-baci,-mpa_size,-f,-f_v_m,-u,-final_u,-m) %>% 
  na.omit()

huh <- lm((pop_effect) ~ ., data = a %>% select(pop_effect,fleet_model,larval_movement, adult_movement, mpa_reaction, steepness, density_movement_modifier, density_dependence_form,sprinkler, mpa_reaction))


d <- recipes::recipe(pop_effect ~ ., data = a) %>% 
  step_dummy(all_nominal())
...
a = fishery_outcomes %>% 
  ...
  ungroup() %>% 
  select(-experiment,-msy,-b_msy,-year,-mpa_size1,
         -`no-mpa`,-`with-mpa`, -years_protected,-depletion,-mpa_effect,-scientific_name,-fishery_effect,-fleet_model) %>% 
  na.omit()


d <- recipes::recipe(msy_effect ~ ., data = a) %>%
  step_dummy(all_nominal())
...
what do we want from mpas?
  - more and more focus is on regional effects

What do we mean by a regional effect?
  - demonstrate causal effect

What does theory say these regional effects should be?



How do we detect it?
  - if theory says things should maybe be small, how do we square that with the 400% increases in lester, sala, etc. 
  

so that's all well and good, what evidence is there supporting all this theory?
lester, halpern, sala, etc. 
Roberts, Fletcher, the GoM logbook example 

# what causes negative effects?

```{r}
cc_importance_plot <- outcomes %>%
  group_by(experiment) %>% 
  filter(years_protected == max(years_protected), fleet_model == "constant-catch") %>% 
  ggplot(aes(f_v_m, depletion, color = mpa_effect <0 )) + 
  geom_point(show.legend = TRUE) 

cc_importance_plot <- outcomes %>%
  group_by(experiment) %>% 
  filter(years_protected == max(years_protected), fleet_model == "constant-catch", f_v_m < 1) %>% 
  ggplot(aes(f_v_m, adult_movement, color = mpa_effect <0 )) + 
  geom_point(show.legend = TRUE) 
  
  fishery_outcomes %>% 
    filter(experiment == 2418) %>% 
    select(year, `with-mpa`, `no-mpa`) %>% 
    gather(scenario, biomass, -year) %>% 
    ggplot(aes(year,biomass, color = scenario)) + 
    geom_line()
  
  b <- a$mpa_experiment[[1]]$raw_outcomes
  
  b %>% 
    group_by(experiment,year, mpa) %>% 
    summarise(tc = sum(biomass_caught),
              tb = sum(biomass),
              percent_mpa = mean(mpa)) %>% 
    ggplot(aes(year, tb, color = mpa)) + 
    geom_point() + 
    facet_wrap(~experiment)
  ...
  processed_grid %>% 
    filter(fleet_model == "constant-catch") %>% 
    select(experiment, f_v_m, year_mpa, fishery_effect) %>% 
    filter(experiment == 8585) %>% 
    unnest() %>% 
    gather(scene, catch, `no-mpa`:`with-mpa`) %>% 
    ggplot(aes(year, catch, color = scene)) + 
    geom_vline(aes(xintercept = min(year) + year_mpa)) +
    geom_point()

```

# figures for MPA simulation

plots are looking good, better off pulling up a saved simulation
...
gganimate::anim_save(animation = doh,filename = "doh.gif")
...
raw_sum %>%
  ungroup() %>%
  ggplot() +
  geom_line(aes(year, value, color = experiment), size = 1) +
...
# do the raw values, leave a line for the no-MPA values
# 
# doh <- raw %>%
#   filter(metric == "biomass") %>%
#   select(-delta, -ref) %>% 
#   gather(experiment,value, `no-mpa`:`with-mpa`) %>% 
#   ggplot() +
#   geom_col(
#     aes(patch, value, fill = experiment),
#     alpha = 0.75,
#     width = 1,
#     position = "dodge"
#   ) +
...
ok so you have your first version of a BACI up and running... And the results look meh. or rather, currently if BACI says high density increase, then overall density increase is likely to be positive, but order of magnitude lower. 

currently calculating baci as (mpa after - mpa before) - (outside after - outside before). is that really right? So basically the critique there would be that if you thought that the outside was a control (e.g. same habitat), but it's actually affected by the treatment, then you're likely to get the overall sign of the effect but completely miss the magnitude. Is that a fair comparison though? Since you're saying the "correct" measure is the system wide density, and they're saying that no, that's not even what you're trying to measure, if you're doing a BACI you're tryign to measure the effect inside the MPAs themselves. So, in that case, the corect comparison is density inside the MPAs with treatment and density just inside the MPAs withot treatment? Can mess with it a bit more, but let's spend tomorrow focused on retooling the paper and you can see where this analysis fits in story wise before wasting time changing analyses you don't use. 

Also, give some thought and care in the paper to differences of densities and biomass
