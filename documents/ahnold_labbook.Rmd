...
run_dir <- "v1.0"
load(
  file = here::here("results",run_dir,"rawish_ahnold_data.Rdata"))
...
tester <- pisco_data %>%
  left_join(site_data, by = c("site",'side')) %>% 
  filter(region %in% c("SMI","SRI","SCI","ANA")) %>% 
  mutate(biomass = 0.001*fish_tl ^3) %>% 
...
tester %>% 
  ggplot(aes(year, mb, color = classcode)) + 
  geom_line(show.legend = F) + 
  facet_wrap(~region)
  
simple <- lm(log(mb) ~ targeted + factor_year + targeted:factor_year, data = tester %>% filter(year > 1999))

a = simple %>% 
  broom::tidy() %>% 
  filter(str_detect(term, "targeted:"))

test <- stan_glmer(log(mb) ~ (1|classcode) + (factor_year - 1|targeted) - 1, data = tester, chains = 1)

of_interest <- test$coefficients[ str_detect(names(of_interest),"targeted:")]
