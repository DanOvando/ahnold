...
fishery_outcomes <- readRDS(file.path(run_dir,"fishery_outcomes.rds"))
...
```{r conservation-effect, fig.cap = "Median (A) and range (B) of regional MPA conservation effects (expressed as percent changes in biomass with MPAs relative to what would have occured without MPAs) after 15 years of protection. For (A), X-axes indicate the depletion of the fishery, where depletion is the percentage of unfished biomass that has been removed from the population, and Y-axes is the percent of the population's range encompasssed inside an MPA. For B), y-axes show the regional conservation effect.", include = FALSE}
...
\begin{figure*}%[tbhp]
  ...
  \caption{Median (A) and range (B) of equilibrium MPA catch effects, expressed as the percentage difference in catch with and without MPAs, across a range of depletion and MPA sizes (and incorporating the full range of scenarios included in our study except for constant-catch). "Range in MPA" is the percent of patches covered by an MPA, "Depletion" is the depletion that would have occured in equilibrim without the MPA}
...
baci_plot <- density_ratios %>%
  ...
  mutate(adult_movement = adult_movement / max(adult_movement)) %>%
  filter(years_protected == max(years_protected)) %>%
  ggplot(aes(y = pmin(2, mpa_effect), x = pmax(-1,pmin(2, baci - 1)))) +
    geom_hline(aes(yintercept = 0)) + 
  geom_vline(aes(xintercept = 0)) + 
  geom_smooth(method = "lm", se = FALSE) +
  geom_abline(aes(slope = 1, intercept = 0), linetype = 2, size = 1.5, color = "red") +
...
\begin{figure*}%[tbhp]
  ...
  \caption{Simulated response ratios (x-axis) plotted against true regional change in biomass caused by MPAs (y-axis). Each point represents a simulation, and color represents the movement rate of fish in the simulation (expressed as a percentage of the patches a fish is likely to move in one time step). The black line shows the one:one fit line, the blue line is a linear fit of the relationship between observed response ratios and true regional change in biomass}
  \label{density-ratio}
\end{figure*}
