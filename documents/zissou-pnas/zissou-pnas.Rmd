...
bad_sims <- processed_grid %>%
  ...
  summarise(bad = any(depletion > 0.9 |
                        abs(pop_effect) > 1e-3)) %>%
  filter(bad == TRUE)
...
Much of the literature on the effects of Marine Protected Areas focuses on assessing the conservation effects within the borders of protected areas [e.g. @edgar2014]. While these effects are extremely important, they do not paint a complete picture of the effects of MPAs. The organisms within the borders of protected areas are generally part of a broader population, the biological stock, connected through adult movement and larval dispersal. If the goal of conservationists or natural resource managers is to increase the total abundance or productivity of a resource, a broader question we should ask of MPAs is not just are there more fish inside the borders of a reserve, but how have the reserves affected the region in which they are located?
...
We used our simulation model to explore the effects of MPAs on fisheries. We express the fishery effects as a percentage change in total catches in the scenario with MPAs relative to the scenario without MPAs. MPAs produced gains in fishery catches in `r percent(pos_fish_runs, accuracy = 1)` of simulations (meaning MPAs produced reductions in fishery catches in `r percent(1 - pos_fish_runs, accuracy = 1)` of simulations). The median MPA effect for MPAs covering less than 25% of the target species' range was negligible. MPAs covering 50% or more of the range generally produced reductions of catches of 25% or greater, though large MPAs were able to provide fishery benefits of 25% or higher when the target fishery was severely depleted in the absence of MPAs (Fig. \ref{catch-msy-effects}A). Under the assumptions of our model MPAs were only able to produce fishery benefits when without-MPA depletion was greater than roughly 60%, though even then while positive outcomes were more frequent negative ones were still possible (Fig. \ref{catch-msy-effects}B). The simulated fishery effects of MPAs, similar to the conservation outcomes, are highly variable and context dependent.
...
dr_plot <- density_ratios %>%
  ...
  scale_color_viridis(guide = gc, labels = scales::percent_format(accuracy = 1), name = "Fish Movement",
                      option = "cividis") + 
...
The raw trends in the densities appear to show parallel trends in the densities of targeted and non-targeted fishes pre-MPA, with the targeted group diverging first positively and then negatively over time (Fig.\ref{pop-trends}). However, these raw trends could could be caused by any number of confounding variables besides the effects of the MPAs themselves, from changes in observer skill and sampling locations, to shifts in environmental conditions. We used our difference-in-difference style regression to attempt to account for some of these confounding factors, and estimate the difference in the biomass densities of targeted and non-targeted fishes in the Channel Islands post-MPA implementation attributable to the MPAs. We found no significant difference in the densities of targeted and non-targeted fishes pre-MPA (before 2003), which provides support for the parallel trends assumption critical to the difference-in-difference method. Following the implementation of the MPAs in 2003, we see evidence of an increasing trend in densities of targeted fishes relative to non-targeted, reaching a peak value in 2014. Following 2014 though, densities of targeted species appear to have declined relative to what we would expect based on the non-targeted group (Fig.\ref{did-plot}).
...
MPAs are an important part of the marine resource management toolbox. Under ideal circumstances they can protect individual species and ecosystem linkages, while supporting local economies through tourism and fishing opportunities. However, our results show that regional conservation and fishery benefits of MPAs are highly context dependent. While we cannot assign probabilities to our simulated states of nature, most of our simulations produced conservation gains on the order of 0% to 25% and negative fishery effects. 

Our simulation results present a clear outcome from the perspective of fishery effects: Under the assumptions of our model, MPAs should not be expected to provide improvements in fisheries catches unless the stock is experiencing substantial overfishing (Fig.\ref{catch-msy-effects}-B). This general result has been discussed before [@hilborn2004], but our results provide the most comprehensive simulation testing of this concept to date.  But, MPAs placed in overfished fisheries frequently produced catch gains of 0-25%, though larger and smaller effects were still possible. MPA size also presents an interesting tradeoff from a fisheries perspective. "Small" MPAs (less than 25% of the population range) very rarely produced substantial fishery losses, but also never produced substantial fishery gains. Medium sized MPAs (25%-50%) produced the potential for higher fishery gains, but also opened up risk of greater fishery looses. For MPAs covering more than 50%, fishery losses quickly become much more likely than fishery gains.

We only measure one aspect of fishery outcomes here (catches). MPAs can affect many other outcomes of importance to fisheries, include profits (e.g. if institutions do not create derby fishing and MPAs improve catch per unit effort along the borders of reserves), resiliency, and research. Our model does not account for potentially important factors such as inter-species effects. Our results though suggest a relatively simple rule of thumb: unless there is clear evidence of overfishing, MPAs should not be expected to produce improvements in fishery catches without clear explanation for the mechanism for these increases (e.g. improvements in habitat). In assessing the potential fishery benefits of MPAs, stakeholders must make explicit the mechanisms by which these benefits are expected to be produced (for example through increased catchability of fish near MPA borders), and what evidence exists for the existence of those mechanisms within the local context.

MPAs produced regional conservation gains in `r percent(pos_runs, accuracy = 1)` of simulations (Fig.\ref{conservation-effects}). MPAs covering less than 25% of the population range were unlikely to produce substantial fishery harm, but also unlikely to produce regional conservation gains above 10% unless the stock would have been severely overfished without the MPA. Large MPAs protecting highly depleted stocks almost always produced large regional conservation gains, as should be expected. The median conservation effect of `r  round(100*median(eqo$mpa_effect))` stands in contrast to the large within-MPA effects on biomass densities reported by @halpern2003a and @lester2009. This different is due in part to to the fact that within-MPA effects are likely to be much larger than regional-MPA effects, but may also reflects the real challenges of translating density ratios into regional conservation effects. Our simulation analysis shows that without proper controls (in particular for the effects of fleet redistribution), density ratios well over 100% are entirely plausible even when the actual conservation effect is much lower. 
...
What do our results say about the future of MPA science? Our simulation model is by no means exhaustive (ignoring features such as species interactions and habitat effects), but captures many of the core factors theorized to affect MPA performance. Our results show that while attributes such as MPA size and fishing pressure are important factors in determining the effects of protection, local fleet dynamics and the movement rates of adult fish can dramatically affect the outcomes of protected areas as well (Fig.\ref{conservation-effects}-\ref{catch-msy-effects}). Far from being a simple tool with clear outcomes then, the effects of MPAs can be highly context dependent, requiring we would argue a bio-economic model of at minimum the complexity presented here to help communities design and set expectations of MPAs. While users may not be able to parameterize every aspect of a model such as that presented here, working with stakeholders to visualize the implications of for example different fleet responses to MPA implementation is preferable to sweeping these critical traits aside through simplified but unrealistic modeling (interested readers can explore outcomes from our simulation model through an interactive web application available [here](XX)). 
...
\begin{equation}
  E_{t} = E_{t-1}\times\theta{PPUE{t-1}}
  \label{eqn:oa}
\end{equation}
e
and
...
\begin{equation}
  PPUE{t} = \frac{\sum_{i=1}^{p}Catch_{i,t} - cE_{i,t}^\beta}{\sum_{i=1}^p E_{i,t}}
  \label{eqn:profits}
\end{equation}
