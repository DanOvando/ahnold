...
\begin{equation}
n_{t,a = 1,p} = \left(\frac{0.8{\times}r0_{p}\times{h}\times{larv_{t,p}}}{0.2\times{ssb0_p}\times(1 - h)+(h - 0.2)\times{larv_{t,p}}}\right)\times \boldsymbol{d^l} \times \epsilon_t
  (\#eq:dd3)
\end{equation}
...
The simplified explanation of the estimation is a hierarchichal model in which we first standardize the observed biomass densities into an abundance index of each species over time. The abundance indices in each year are assumed to be log-normally distributed with means and standard deviations for the targeted and non-targeted groups, giving an estimate of the mean densities of targeted and non-targeted species over time. We then calculate the difference between mean density of targeted species and the mean density of non-targeted species in each year.
...
Species are indicated by "classcode". We provide a table linking classcodes to scientific name, common name, and whether the species is listed as targeted by fishing (Table.S\@ref(tab:classcode))


```{r classcode, include=FALSE}

classcode_table <-
  abundance_data$data[[which(abundance_data$data_source == "pisco")]] %>%
  select(classcode, taxa, commonname, targeted) %>%
  unique()

knitr::kable(classcode_table,
             digits = 2,
             caption = "Classcode, scientific name, common name, and targeted marker for species included in difference-in-difference regression.",
             longtable = TRUE,
             booktabs = TRUE,
             col.names = colnames(classcode_table)) %>%
